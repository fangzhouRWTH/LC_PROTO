...
class OrcaPedestrianDynamicAgentBackend:
    """Mock ORCA pedestrian backend for runtime validation."""
    ...
    def build_from_plan(self, plan: DynamicScenePlan | None):
        ...
        warnings: list[str] = []
        planner_states = build_pedestrian_states_from_plan(self.plan, warnings)
        for warning in warnings:
            print(f"[WARN] {warning}")

        for actor_plan in self.plan.actors:
            if actor_plan.actor_type != "pedestrian":
                print(
                    f"[WARN] orca_pedestrian backend skips non-pedestrian actor: "
    ...
    def spawn(self, stage=None):
        ...
        self.reset()
        print(f"[OK] Spawned {len(self.actors)} ORCA pedestrian actor(s).")
