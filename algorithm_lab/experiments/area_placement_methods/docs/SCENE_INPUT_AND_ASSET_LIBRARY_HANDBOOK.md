...
- **扩展区域名（推荐）**：`placeholder_area_publicspace_<index>_<typecompact>[_<boundarycompact>]`
  - typecompact 无下划线，如 `blockentrance`、`citystreetroofless`
  - boundarycompact 可选，如 `streetboundaryprimary`、`buildingentrancemain`
  - 解析器优先读 `simworld:public_space_type`，缺失时用名称；边段缺失时按 type + hint 自动合成
- 仍支持 legacy 下划线类型名（如 `..._city_yard_roofless`）

### 1.4.1 公共空间区域（必填）

| 项 | 值 |
| --- | --- |
| **Prim 名称** | `placeholder_area_publicspace_<index>_<typecompact>[_<boundarycompact>]`（见上） |
...
| 属性名 | 类型 | 必填 | 合法值 |
| --- | --- | --- | --- |
| `simworld:public_space_type` | string / token | **是*** | [§1.3.1](#131-public_space_type公共空间类型) 之一；*若 prim 名为 `..._<public_space_type>` 可省略 |
| `simworld:ratio_dynamic_static` | float | 否 | `0.0` … `1.0`；缺失时解析默认 `0.36` |
...
**无子边段时（运行时 / USD 补全脚本）：** 若区域下没有 ≥3 条有效 `placeholder_segment_edge_*`，且 mesh 为四边形，引擎按 `public_space_type` 模板 + 可选 boundary hint 合成 4 条边。持久化写入 USD：

```bash
scripts/ensure_public_space_usd.sh --usd path/to/scene.usd
# 预览：加 --dry-run；覆盖已有边段：--overwrite-segments
```
...
| `public_space_type` | `simworld:public_space_type` 或 prim 名后缀 `..._<type>` |
| `public_space_segments[]` | 子 prim `placeholder_segment_edge_*`；不足时由四边形 mesh 推断 |
